...
function buildTrainingWeek(state,week=monday(),force=false){
 ...
 if(!force&&existing?.engineVersion==='6.1.0')return existing;
 ...
 for(let i=0;i<5;i++){
  ...
  addp(i%2?pools.backH:pools.backV,'back');
 ...
 state.trainingPlans[week]={week,engineVersion:'6.1.0',generatedAt:new Date().toISOString(),days};
